...
class FedLeaOffice31(FederatedDataset):
    NAME = 'fl_office31'
    SETTING = 'domain_skew'
    DOMAINS_LIST = ['amazon', 'dslr', 'webcam']
    percent_dict = {'amazon': 0.48, 'dslr': 0.48, 'webcam': 0.48}
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), FedLeaOffice31.Nor_TRANSFORM])
        return transform
    ...
    def get_backbone(parti_num, names_list):
        ...
        if names_list == None:
            for j in range(parti_num):
                nets_list.append(resnet12(FedLeaOffice31.N_CLASS))
        else:
            for j in range(parti_num):
                net_name = names_list[j]
                nets_list.append(nets_dict[net_name](FedLeaOffice31.N_CLASS))
        return nets_list
