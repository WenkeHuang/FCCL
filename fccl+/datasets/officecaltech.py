...
class FedLeaOfficeCaltech(FederatedDataset):
    ...
    SETTING = 'domain_skew'
    DOMAINS_LIST = ['amazon', 'caltech','dslr','webcam']
    percent_dict = {'amazon': 0.48, 'caltech': 0.48, 'dslr': 0.48,'webcam':0.48}
    ...
    def get_backbone(parti_num, names_list):
        ...
        if names_list == None:
            for j in range(parti_num):
                nets_list.append(resnet12(FedLeaOfficeCaltech.N_CLASS))
        else:
            for j in range(parti_num):
